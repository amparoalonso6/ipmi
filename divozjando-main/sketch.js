...
function draw() {
  ...
  // Graves y Agudos
  //let graves = fft.getEnergy(20, 400);
  //let agudos = fft.getEnergy(400, 20000);

  

  //console.log("graves",graves);
  //console.log("agudos",agudos);
  //console.log("centroide",fft.getCentroid());
  ...
  // Calcular color base según volumen (0 = frío, 0.3+ = cálido)
...
//console.log("vol",volumenSuavizado);
fft.analyze();
if(volumenSuavizado > umbralRuido){
  if(fft.getCentroid()>umbralFrioCaliente ){
    console.log("frios")
    console.log(fft.getCentroid())
    colorHueBaseMem = random(50,230);
  }else{  
    console.log("calidos")
    console.log(fft.getCentroid())
    let cn = random(1)>0.5?random(0,50):random(230,360);
    colorHueBaseMem = cn;  
  }
}
//console.log(colorHueBase)
...
colorHueBase = constrain(colorHueBase, 0, 360);
  ...
  let saturacionDinamica = map(volumenSuavizado, 0, 0.25, 80, 255);
...
let h = (colorHuePorCapa[r]+colorHueBase)%360;//lerp(colorHuePorCapa[r], colorHueBase, mezclaFactor);
